  ...
  <div>
    <v-list-item two-line>
      <v-card ref="card" @mouseleave="mouseLeave" @mousemove="mouseHover('start', $event)" :width="getWidthFunctionContainer" :min-width="getWidthFunctionContainer" :style="cssVars" :color="getColor" height="55px" @contextmenu="contextMenuDM" @click="select">
        <v-row align="center" align-content="center" class="mt-2">
          <h2 class="ml-6 mr-5" style="user-select: none;"> For : </h2>
          <vsm-setter :initialval="false" :variable="functionAction.inputs[0]" :assets="assets" :listvar="assets[5].content" :onlyVariable="true"></vsm-setter>
          <h2 class="ml-6 mr-5" style="user-select: none;"> from </h2>
          <vsm-setter :initialval="false" :variable="functionAction.inputs[1]" :assets="assets" :listvar="assets[5].content"></vsm-setter>
          <h2 class="ml-6 mr-5" style="user-select: none;"> to </h2>
          <vsm-setter :initialval="false" :variable="functionAction.inputs[2]" :assets="assets" :listvar="assets[5].content"></vsm-setter>
          <v-spacer></v-spacer>
    ...
    <v-list-item>
      <v-card ref="cardEnd" @mouseleave="mouseLeave" @mousemove="mouseHover('end', $event)" :width="getWidthFunctionContainer" :min-width="getWidthFunctionContainer" :style="cssVars" :color="getColor" height="30px" @contextmenu="contextMenuDM" @click="select">
        <h2 class="ml-2 mr-5 mt-1" style="user-select: none;"> End for loop </h2>
      </v-card>
    </v-list-item>
  </div>
</template>
